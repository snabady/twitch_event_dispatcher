...
class ChatStats():
    ...
    async def add_channel_join(self, user_name: str):
    ...
    async def print_unique_viewers(self):
        #print(f"unique_viewers: {self.unique_viewers}")  
        for user, time in self.unique_viewers.items():
            print(f"{user}: {time}")


    async def get_view_count(self):
        return len(self.unique_viewers)

    async def write_stats_history(self):
    ...
    async def init_stats(self):
        ...
        print(type(lastline[self.stats_history_csv_map['total_msg']]))
        print(lastline[self.stats_history_csv_map['total_msg']])
        self.total_msg              = int(lastline[self.stats_history_csv_map['total_msg']])
        self.total_channel_joins    = int(lastline[self.stats_history_csv_map['total_channel_joins']])
        self.total_follower         = int(lastline[self.stats_history_csv_map['total_follower']])
        self.total_subs             = int(lastline[self.stats_history_csv_map['total_subs']])
