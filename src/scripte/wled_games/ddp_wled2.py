...
def measure_text(text: str, font: ImageFont.ImageFont):
    # Messe Textgröße robust: benutze textbbox wenn verfügbar, sonst textsize
    ...
    try:
        ...
        # Fallback für sehr alte Pillow-Versionen
        w, h = draw.textsize(text, font=font)
    return w, h

def render_text_bitmap(text: str, font_path=None):
    # Versuche eine kleine 8px-TTF, sonst default font
    if font_path and os.path.exists(font_path):
        font = ImageFont.truetype(font_path, 8)
    else:
        font = ImageFont.load_default()

    # Ermittle Textbreite und -höhe via ImageDraw (avoid font.getsize())
    ...
    # Vertikal zentrieren: wenn draw.textbbox liefert negative top, das ignorieren wir hier
